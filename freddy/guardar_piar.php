...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $codigo_piar = isset($_POST['CODIGO_PIAR']) ? $conn->real_escape_string($_POST['CODIGO_PIAR']) : '';
    ...
    if (!empty($codigo_piar) && !empty($evaluacion) && !empty($flexibilizacion) && !empty($barra_aprendizaje) && !empty($periodo) && !empty($dba)) {
        ...
        $sql = "INSERT INTO piar (CODIGO_PIAR, EVALUACION, FLEXIBILIZACION, BARRA_DE_APRENDIZAJE, PERIODO, DBA) 
                VALUES ('$codigo_piar', '$evaluacion', '$flexibilizacion', '$barra_aprendizaje', '$periodo', '$dba')";
